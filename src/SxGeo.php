...
class SxGeo
{
     ...
     */
    public function __construct(string $db_file, int $type = SXGEO_FILE)
    {
    ...
    public function getCityName(string $ip, string $locale = 'ru')
    {
        $seek = $this->get_num($ip);

        if($cityInfo = $seek ? $this->parseCity($seek) : false) {
            return $cityInfo['city']["name_$locale"] ?? null;
        }
    }
